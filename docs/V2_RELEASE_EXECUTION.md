...
> 最近核对：2026-08-19
>
> 当前结论：v2.0 发布范围已正式限定为 Local + Direct SSH/SFTP；本地 A～D、H 门禁、Direct/TUN 真实候选链路和 required Contracts/Rust/Release Actions 已通过。ProxyCommand 实现与测试保留为 post-v2 延后项；阶段 I 由 owner 明确豁免但未执行生产验收，PR #30 已合入冻结候选并完成合并后 required 门禁，`main` 合并和正式发布仍未完成。

### 本轮接手核对（2026-08-19）

- 本地/远端独立工作分支：`release/v2-rc`，从 `origin/feat/v2-m1-backend-config` 创建；PR 验证 head 为 `5a04dba59d839e99fd6425a31a6ddea4e9941ba5`，已合入冻结候选并生成 merge commit `4242e9d0ee60d8b2c54c78f59d254a355c52b250`（短 SHA：`4242e9d`）。冻结候选原始 SHA 仍为 `848ae2d04a4f78d1acbe7d4708dbd48e4b183e1b`。
- `origin/main` 仍为 `ce2abf108da6c7e8e709ddbaa3992066807f83c3`；远端候选分支已由 PR #30 更新为 `4242e9d0ee60d8b2c54c78f59d254a355c52b250`。
- 工作区原有用户修改已保留：`README.md` 的文档索引变更和本手册文件；未覆盖或丢弃。
- PR #25：open、Draft、未合并，head/base 与上述候选和 `main` 一致。
- 独立 scope PR #30：closed、Ready、已合并，head=`release/v2-rc`/`5a04dba`，base=`feat/v2-m1-backend-config`/`848ae2d`，merge commit=`4242e9d`；`main` 尚未修改。
- Issue #23：open；Billing/Spending Limit 阻塞已解除，但按手册保持打开，直到目标验收和最终发布门禁完成。
- Issue #26：已由本轮 scope decision 关闭，state reason=`not_planned`；原因是 ProxyCommand 已移出 v2.0 范围，不代表功能 PASS。Issue #27：open；owner 已明确本 RC 不需要 `main` branch protection，并接受手册定义的 procedural fallback；PR #30 已完成合并，但这不等于 tag/release 授权。
...
- 独立 PR #30 的 required Actions 已在 public 仓库标准 `ubuntu-latest` runner 上真实执行并通过：Contracts `32224992356`、Rust `32224992337`、Release `32224992311`；合并 commit `4242e9d` 的合并后 required Actions 也已通过：Contracts `32227671507`、Rust `32227671538`、Release/package `32227671389`。对应 checkout、fmt/Clippy、Rust tests/build、transport smoke、healthcheck、upgrade/rollback、package/checksum 步骤均有成功记录。Release 的 tag-only publish job 按预期 skipped。ProxyCommand/M7 旧 workflow 不属于 v2.0 required gate。
...
origin/feat/v2-m1-backend-config  4242e9d0ee60d8b2c54c78f59d254a355c52b250
Draft PR                          #25
独立 scope PR（已合并）           #30 (`release/v2-rc` -> `feat/v2-m1-backend-config`)
外部阻塞 Issue                    #23
```
...
- 状态：`PASS`。
- 证据：公开仓库后的 PR [#30](https://github.com/liqiangcc/log-query-mcp/pull/30) required Actions 已真实执行：Contracts `32212739893`、Rust `32212739852`、Release `32212739935` 全部 `success`，不再出现 Billing 启动前失败 annotation。Rust job 真实完成 checkout、fmt、Clippy、tests 和 release build；Release job 真实完成 transport smoke、healthcheck、upgrade/rollback、package/checksum validation。
- PR [#25](https://github.com/liqiangcc/log-query-mcp/pull/25) 仍为 Draft，Issue [#23](https://github.com/liqiangcc/log-query-mcp/issues/23) 保持 open；后者仍需等待目标验收和最终发布门禁，不因 Actions 恢复而提前关闭。
- 阶段记录 commit：`5392d71f0421b456517a9be4c6e20504868a93d6`（独立 scope 分支；候选 `848ae2d` 未改动）。
...
- 状态：`BLOCKED / 未进入 main 合并与发布操作`；阶段 A、G 已完成，阶段 I 已由 owner 豁免但未执行，PR #30 已完成 review 门禁范围内的合并并生成 `4242e9d`，合并后的 Contracts/Rust/Release required Actions 已通过；但 `main` 合并、独立 tag/release 授权仍未完成，不能直接创建 tag/release 或部署。Issue #27 的 protection fallback 已按 owner 决策接受，不再作为阻塞；ProxyCommand helper/Issue #26 不再是 v2.0 硬门禁，已转为 post-v2 跟踪。
- 独立 `release/v2-rc` 的验证 head=`5a04dba` 已合入 `feat/v2-m1-backend-config`，merge commit=`4242e9d`；本次后续文档提交只记录合并状态，不修改 `main`，不执行 tag/release/deploy。
