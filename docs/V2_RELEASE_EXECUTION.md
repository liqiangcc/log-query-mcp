...
- Windows 主机 `wsl --status` 显示默认发行版 `Ubuntu-22.04`、默认 WSL 版本 `2`、kernel `5.10.16`，但 `wsl --version` 返回“命令行选项无效”。这确认当前是旧版/inbox WSL CLI；仅重启当前实例不能证明 systemd 支持，需先将 WSL 组件更新到支持 systemd 的版本，再执行 `wsl --shutdown` 后复验。
