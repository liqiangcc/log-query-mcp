...
- `/etc/log-query-mcp/config.json` 及备份仍为旧 v1 本地 source；未覆盖、替换或重启旧 `v0.1.0` 服务。上述是 service identity 下的隔离候选进程验证，不是 production systemd MainPID 验收；原 M7 ProxyCommand 和完整真实目标 systemd HTTP gate 仍保持未执行。
