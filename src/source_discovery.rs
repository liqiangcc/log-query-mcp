...
use crate::{SafeOpenError, SafeRoot};

pub const MAX_DIRECTORY_RULES_PER_SOURCE: usize = 32;
...
impl DirectoryDiscoveryRule {
    pub fn new(
        ...
        for suffix in include_suffixes {
            ...
            {
                return Err(SourceDiscoveryError::InvalidRule(
                    "include suffix is empty, too long, or contains a path separator",
                ));
            }
    ...
    for rule in rules {
        ...
        while let Some(directory) = pending.pop() {
            ...
            for (name, file_type) in entries {
                ...
                // Some filesystems report DT_UNKNOWN. Classify only through
                // safe openat2-based operations; never follow the entry path.
