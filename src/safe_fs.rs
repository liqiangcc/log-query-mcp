...
impl SafeRoot {
    ...
    /// symlink. The retained descriptor anchors all later path resolution to
    /// the exact directory object approved at startup.
    ...
    /// Opens a regular file below the configured root.
    ///
    /// The kernel rejects parent traversal, absolute paths, symlinks, magic
    /// links, and crossing into a nested mount or bind mount while resolving
    /// the relative path.
    ...
    /// Opens a directory below the configured root for fd-relative discovery.
...
fn validate_file_path(path: &Path) -> Result<(), SafeOpenError> {
    validate_normal_relative_path(path, false)
}

fn validate_directory_path(path: &Path) -> Result<(), SafeOpenError> {
    validate_normal_relative_path(path, true)
}

fn validate_normal_relative_path(path: &Path, allow_single_dot: bool) -> Result<(), SafeOpenError> {
    if allow_single_dot && path == Path::new(".") {
        return Ok(());
    }
...
mod tests {
    ...
    #[test]
    fn resolve_policy_includes_cross_mount_restriction() {
