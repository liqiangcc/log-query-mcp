...
impl SafeRoot {
    /// Opens an administrator-approved source root without following a final
    /// symlink. The descriptor remains the anchor for all later path lookups.
    ...
    /// Opens a regular file below this root. Linux resolves the complete path
    /// with the v1 no-escape, no-link, and no-cross-mount policy.
    ...
    /// Opens a directory below this root for fd-relative discovery.
...
fn validate_relative_file_path(path: &Path) -> Result<(), SafeOpenError> {
    validate_relative_path(path, false)
}

fn validate_relative_directory_path(path: &Path) -> Result<(), SafeOpenError> {
    validate_relative_path(path, true)
}

fn validate_relative_path(path: &Path, allow_dot: bool) -> Result<(), SafeOpenError> {
...
pub enum SafeOpenError {
    ...
    #[error("configured path must be normalized and relative to its source root")]
    InvalidRelativePath,
...
mod tests {
    ...
    #[test]
    fn v1_resolve_policy_includes_cross_mount_rejection() {
    ...
    fn opens_root_and_nested_directories() {
        ...
        root.open_directory_fd(".")
            .expect("source root should open for discovery");
        root.open_directory_fd("archive/day-1")
            .expect("nested directory should open for discovery");
    }

    #[test]
    fn rejects_parent_traversal_absolute_paths_and_dot_file_paths() {
        let root_dir = tempdir().expect("temporary root should be created");
        let root = SafeRoot::open(root_dir.path()).expect("root should open");
    ...
    #[test]
    fn rejects_directory_fifo_and_socket_as_log_files() {
        let root_dir = tempdir().expect("temporary root should be created");
