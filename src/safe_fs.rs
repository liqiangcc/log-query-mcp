...
impl SafeRoot {
    /// Opens an administrator-configured source root without following a final
    /// symlink and retains its directory descriptor for later relative opens.
    ...
    /// Opens a regular file relative to this source root.
    ///
    /// Linux resolves the complete path with `openat2()`. Parent traversal,
    /// absolute paths, symlinks, magic links, and nested mount crossings are
    /// rejected by the kernel rather than by a string-prefix check.
    ...
    /// Opens a directory relative to this source root for bounded discovery.
...
fn validate_relative_path(path: &Path, allow_root_dot: bool) -> Result<(), SafeOpenError> {
    if allow_root_dot && path == Path::new(".") {
        return Ok(());
    }
...
pub enum SafeOpenError {
    ...
    #[error("configured object path must be normalized and relative to its source root")]
    InvalidRelativePath,
    ...
    #[error("configured log directory cannot be opened safely")]
    DirectoryOpen {
        #[source]
        source: Errno,
    },
...
mod tests {
    ...
    #[test]
    fn resolve_policy_includes_v1_cross_mount_protection() {
    ...
    #[test]
    fn opens_root_and_nested_directories() {
        let root_dir = tempdir().expect("temporary root should be created");
        ...
        root.open_directory_fd(".")
            .expect("root should open for discovery");
        root.open_directory_fd("archive/day-1")
            .expect("nested directory should open for discovery");
    }

    #[test]
    fn rejects_parent_traversal_absolute_and_dot_paths() {
        let root_dir = tempdir().expect("temporary root should be created");
        let root = SafeRoot::open(root_dir.path()).expect("root should open");

        for path in ["../outside.log", "/etc/passwd", "./application.log"] {
            assert!(matches!(
                root.open_regular_file(path),
    ...
    #[test]
    fn rejects_directories_fifo_and_socket_as_files() {
        let root_dir = tempdir().expect("temporary root should be created");
